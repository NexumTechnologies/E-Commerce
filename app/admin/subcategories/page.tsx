...
async function fetchSubCategories(page: number, size: number) {
  const res = await api.get("/subcategory", { params: { page, size } });
  return res.data;
}
...
export default function AdminSubCategoriesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-subcategories", page, size],
    queryFn: () => fetchSubCategories(page, size),
  });

  const payload = data as PaginatedResponse<SubCategoryItem> | undefined;
  const subCategories = payload?.data?.items ?? [];
  const pagination = payload?.data?.pagination;
  const totalItems = pagination?.totalItems ?? subCategories.length;
          ...
          <>
            ...
            <div className="p-4">
              {subCategories.length === 0 ? (
                <div className="py-8 text-center text-sm text-gray-500">No subcategories found.</div>
              ) : (
