...
export default function AdminBuyersPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const resp = await api.get(`/users`, { params: { role: "buyer", page, size } });
      return resp.data;
    },
    ...
    <div>
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage buyer accounts and approvals.</p>
        </div>
        <div>
          <Link href="/admin/approvals/buyers" className="text-sm text-[#7c3aed]">View Approvals</Link>
        </div>
      </header>
          ...
          <>
              ...
              <tbody>
                {users.map((b: any) => (
                  <tr key={b.id} className="border-t">
                    ...
                    <td className="px-4 py-3">{b.is_varified ? "Yes" : "No"}</td>
                    <td className="px-4 py-3"> <Link href={`/admin/users/buyers/${b.id}`} className="text-sm text-[#7c3aed]">View</Link> </td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="p-4 flex items-center justify-between">
              ...
              <div className="space-x-2">
                ...
                <button
                  disabled={meta.total && page >= Math.ceil((meta.total || 0) / size)}
